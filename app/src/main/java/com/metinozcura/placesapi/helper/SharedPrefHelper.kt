package com.metinozcura.placesapi.helper
...
class SharedPrefHelper(private val sharedPreferences: SharedPreferences, private val gson: Gson) {
     ...
     */
    fun <T> addListData(key: String, list: MutableList<T>?) {
    ...
    fun appendListData(key: String, data: String) {
        val itemType = object : TypeToken<MutableList<String>>() {}
        var list = retrieveListData(key, itemType)
        if (list == null)
            list = mutableListOf()
        list.add(data)
        addListData(key, list)
    }

    /**
     * Retrieve list of objects with given key from Shared preferences
     */
    fun <T> retrieveListData(key: String, tt: TypeToken<MutableList<T>>): MutableList<T>? {

        val json = getStringData(key, null)

        return if (sharedPreferences.contains(key)) {

            if (json == null) null else gson.fromJson<MutableList<T>>(json, tt.type)
